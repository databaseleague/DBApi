    ...
    <div class="api">
              ...
              <el-form-item :label="$t('m.path')">
                <el-input v-model="keyword.path" style="width: 200px" clearable size="mini">
                  <template slot="prepend">
                    <span style="margin-top: 45px;">/{{ context }}/</span>
                  </template>
                </el-input>
              </el-form-item>

              <el-form-item>
                <el-button type="primary" icon="el-icon-search" @click="search" plain size="mini">{{ $t("m.search")
                }}</el-button>
              </el-form-item>
            </el-form>
          </div>
        ...
        <div class="right">
          <el-dropdown @command="" style="margin-right: 15px">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item command="item">
                <span class="el-icon-download" @click="dialogVisible3 = true">{{
                  $t("m.export_api")
                }}</span>
              </el-dropdown-item>
              <el-dropdown-item command="item">
      ...
      <div class="table">
        <el-table :data="tableData" border stripe width="100%" size="mini">
          ...
          <el-table-column :label="$t('m.note')" prop="note"> </el-table-column>
      ...
      <el-dialog :title="$t('m.export_api_groups')" :visible.sync="dialogVisible4" @open="getAllGroups">
        <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange">{{
          $t("m.select_all") }}</el-checkbox>
        <div style="margin: 15px 0"></div>
        <el-checkbox-group v-model="checkList" @change="handleCheckedItemChange">
          <el-checkbox v-for="item in groups" :label="item.id" :key="item.id">{{ item.name
          }}<span style="color: #ccc">{{ item.id }}</span></el-checkbox>
        </el-checkbox-group>
